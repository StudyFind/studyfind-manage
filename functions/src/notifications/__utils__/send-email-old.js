...
module.exports = async (
  ...
  auth,
  uid,
  messageObject = defaultMessageObject,
  isResearcher = true
) => {
  // add unsubscribe message
  messageObject.html +=
    '<h6><a href="https://studyfind.org/settings">Click here to change your email preferences and unsubscribe</a><h6>';

  const [user, userData] = await Promise.all([
    auth.getUser(uid).then((u) => {
      if (!u.email) throw TypeError(`no email exists for user ${uid}`);
      return u;
    }),
    firestore
      .collection(isResearcher ? "researchers" : "participants")
      .doc(uid)
      .get()
      .then((d) => {
        if (!d.exists)
          throw TypeError(
            `'${uid}, is-researcher ${isResearcher}; does not have a firestore entry`
          );
        return d.data();
      }),
  ]);
